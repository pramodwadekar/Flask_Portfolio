...
app = Flask(__name__)
app.secret_key = 'itsaflaskproject'
...
def create_blog():
    if request.method == 'POST':
        ...
        render_template('create_blog.html', all_category_id = global_all_category_no, all_category_name = global_all_category_name)

@app.route('/self_blog/detail/<int:blog_model_id>/<string:blog_model_category>', methods = ['GET','POST'])
@login_required
def  self_blog_detail(blog_model_id, blog_model_category):
    ...
    if request.method == "POST":
        if request.form['action'] == 'Update':
            ...
            BlogModel.query.filter(id = blog_model_id).delete
        db.session.commit()
...
@app.route('/blog_Detail/<int:blog_id>/<string:username>/<string:category', methods = ['GET','POST'])
@login_required
